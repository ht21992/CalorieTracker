...
def get_food_info(request):
    ...
    api_request = requests.get(
        api_url + food_query, headers={"X-Api-Key": settings.API_KEY}
    )
    try:
...
@require_http_methods(["POST"])
def find_excersice(request):
    exc_type = request.POST.get("exc_type", "")
    muscle = request.POST.get("muscle", "")
    difficulty = request.POST.get("difficulty", "")
    if not exc_type or not muscle or not difficulty:
        return HttpResponse("Invalid Parameters")
    api_url = "https://api.api-ninjas.com/v1/exercises?muscle={}&type={}&difficulty={}".format(
        muscle, exc_type, difficulty
    )
    api_request = requests.get(api_url, headers={"X-Api-Key": settings.API_KEY})
    try:
        if not api_request.json():
            return HttpResponse(f"Nothing found for your request")
        resp = json.loads(api_request.content)
    except Exception as e:
        print(e)
        return HttpResponse(f"oops! There was an error")
    ...
    return render(
        request, "partials/excersice_finder/excersice_info.html", {"resp": resp}
    )


@require_http_methods(["POST"])
def recipe_finder(request):
    recipe_query = request.POST.get("query", "")
    if not recipe_query:
        return HttpResponse("Food field is empty")
    api_url = "https://api.api-ninjas.com/v1/recipe?query="
    api_request = requests.get(
        api_url + recipe_query, headers={"X-Api-Key": settings.API_KEY}
    )
    try:
        if not api_request.json():
            return HttpResponse(f"Nothing found for {recipe_query}")
        resp = json.loads(api_request.content)
    except Exception as e:
        print(e)
        return HttpResponse(f"oops! There was an error")
    return render(
        request, "partials/recipe_finder/recipe_info.html", {"resp": resp}
    )
